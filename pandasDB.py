        ...
        for symbol in res_cl_:
            ...
            for codeDict in code_decomp:
                ...
                while code[-1].isnumeric():
                    code = code[:-1]
                if pos == len (code_decomp): # El año lo pillo del ultimo Contrato de la spread
                    try:
                        yearSymbol = int(codeDict['code'][len(code):])
                    except:
                        logging.error('Problema leyendo en año. Codigo %s', codeDict['code']) 
                    else:
                        if yearSymbol < yearInit:
                            yearInit = yearSymbol
                        if yearSymbol > yearEnd:
                            yearEnd = yearSymbol
                if pos > 1:
                    if codeDict['action'] == 'SELL':
                        familyCode += '-'
                    else:
                        familyCode += '+'
                if codeDict['ratio'] > 1:
                    familyCode += str(codeDict['ratio'])
                familyCode += code
    
            if familyCode not in familyList:
                familyList[familyCode] = {'symbols':[],  'nLegs': code_len, 'firstYear': yearInit, 'lastYear': yearEnd}

            familyList[familyCode]['symbols'].append(symbol)
            if yearInit < familyList[familyCode]['firstYear']:
                familyList[familyCode]['firstYear'] = yearInit
            if yearEnd > familyList[familyCode]['lastYear']:
                familyList[familyCode]['lastYear'] = yearEnd

            code_reg = {'symbol': symbol, 'nLegs': code_len, 'family': familyCode}
            new_cl.append (code_reg)

        self.Contracts_ = new_cl
        self.Families_ = familyList

        logging.debug ('----------------------------------')
        logging.debug ('%s', familyList)
        logging.debug ('----------------------------------')
