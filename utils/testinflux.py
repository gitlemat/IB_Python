...
def testToday():
    ...
    param = {"_bucket": "ib_prices", "_start": todayStart, "_stop": todayStop, "_symbol": "HEM3", "_desc": False}
    query = '''
...
def testPnL():
    ...
    print (todayStop)
    param = {"_bucket": "ib_prices", "_start": todayStart, "_stop": todayStop, "_symbol": "HEM3-2HEN3+HEQ3", "_desc": False}
    query = '''
    from(bucket: _bucket)|> range(start: _start, stop: _stop)
...
def testExec():
    ...
    print ("todayStop")
    param = {"_bucket": "ib_prices", "_symbol": "HEM3-2HEN3+HEQ3", "_strategy": "Pentagrama", "_desc": False}
    query = '''
...
def testExecCountSum():
    ...
    print ("todayStop")
    param = {"_bucket": "ib_prices", "_symbol": "HEM3-2HEN3+HEQ3", "_desc": False}
    query = '''
    from(bucket: _bucket)|> range(start: 0)
    |> filter(fn:(r) => r._measurement == "executions")
    |> filter(fn:(r) => r.symbol == _symbol)
